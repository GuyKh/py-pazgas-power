...
class AuthApi:
    async def send_otp(
        ...
        req = PazGasRequest(SEND_OTP_PATH, SendOtpRequest(customer_id=identity_number, customer_phone=phone))

        res = await send_post_json_request(self._state.session, None, APIM_URL, json_data=req.to_dict(), use_auth=False)
        return SendOtpResponse.from_dict(res)

    async def verify_otp(self, session_id, otp) -> CustomerData:
        req = PazGasRequest(VERIFY_OTP_PATH, VerifyOtpRequest(session_id=session_id, otp=otp))
